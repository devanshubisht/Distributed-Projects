...
def main(sig_file, inputs):
	...
	# Updated F-beta check to handle div0
	denominator = foo*num_correct + beta_sq*num_fn + num_fp
	f_beta_score = (foo*num_correct) / denominator if denominator else "N/A"
	if not denominator:
		print("F-beta score not applicable.")
	else:
		print(f"F-beta = {f_beta_score:.4f}")

    # Check if we are in a bonus-compatible setting - xgpg with all CPUs available

	env = os.environ
	slurm_cpus = env["SLURM_JOB_CPUS_PER_NODE"]
	hostname = socket.gethostname()
	print(f"")
	print(f"Checking whether parallel speedup is valid: hostname: {hostname}, num cpus: {slurm_cpus}")
	if hostname.startswith('xgpg') and int(slurm_cpus) == 96: 
		print("Parallel speedup is valid!")
	else:
		print("WARNING - parallel speedup has not been calculated on an xgpg node using all CPUs, so this result is not valid!")
	print(f"Parallel speedup (for bonus, assuming this is a valid run): {parallel_time_ns / cuda_runtime_ns:.3f}x")
	print(f"")
